...
class _OrderSideViewState extends State<OrderSideView> {
  ...
  void initState() {
    super.initState();
    _startCountdown(context.read<CurrencyCubit>().state.lastFetchedAt);
  }
  ...
  void _startCountdown(DateTime? lastFetchedAt) {
    ...
    final elapsed = lastFetchedAt != null
        ? DateTime.now()
              .difference(lastFetchedAt)
              .inSeconds
              .clamp(0, _refreshInterval)
        : 0;
    _secondsUntilRefresh = (_refreshInterval - elapsed).clamp(
      0,
      _refreshInterval,
    );
    _countdownTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted) return;
      setState(() {
        _secondsUntilRefresh = (_secondsUntilRefresh - 1).clamp(
          0,
          _refreshInterval,
        );
      });
  ...
  Future<void> _proceedToPayment(BuildContext outerContext) async {
    ...
    if (tableCubit.state.enabled) {
      // Always route through table selection when the feature is on.
      // Free any stale selection first so the table appears available.
      tableCubit.freeTable(tableCubit.state.selectedTableId);
                    ...
                    children: [
                      ...
                      BlocSelector<HomeBloc, HomeState, (double, double?)>(
                        selector: (state) {
                          ...
                          return (
                            idr,
                            allHaveKas && state.cartItems.isNotEmpty
                                ? kas
                                : null,
                          );
                        },
